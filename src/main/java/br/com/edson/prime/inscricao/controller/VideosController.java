...
import br.com.edson.prime.inscricao.service.VideosService;
import br.com.edson.prime.inscricao.util.Message;


@RestController
@RequestMapping("/adtag/api/videos")
public class VideosController {
	...
	@PostMapping
	public ResponseEntity<?> create(@Valid @RequestBody Videos dto){
		
		

			Videos dtoResponse = service.save(dto);
	...
	@PutMapping
	public ResponseEntity<?> update(@Valid @RequestBody Videos dto){
		
		
		if(dto.getId()>0) {
			Videos m = service.update(dto);
			...
			if(m.getId()>0) {
			 ...
			 return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
					Message.builder()
					.mensagem("Videos inválido").build()
					);
		}
